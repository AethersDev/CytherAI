...
;(function (global) {
  ...
  /* ─── Constants ─────────────────────────────────────────── */
  const MONITOR_DURATION_MS   = 60_000; // Stop observer after 60s — late-loading resources after this window won't be tracked
  ...
  class SealedArtifactMonitor {
    ...
    /**
     * NOTE: Hash is computed from live DOM, so browser extensions that inject
     * elements will produce a different hash. This is a runtime fingerprint,
     * not a reproducible build integrity check.
     *
     * Priority:
    ...
    async _computeBuildHash () {
      ...
      try {
        ...
        console.warn('[SealedArtifact] crypto.subtle unavailable, using FNV-1a fallback');
    ...
    _updateDisplay () {
      ...
      if (this.buildHash) {
        const short = this.buildHash.substring(0, 8).toUpperCase();
        document.querySelectorAll('[data-build-hash]').forEach(el => {
     ...
     */
    getReport () {
      if (!this._initialized || !this.buildHash) {
        return {
          externalRequests: 0,
          externalOrigins:  [],
          buildHash:        '(initializing)',
      ...
      return {
        ...
        sriEnforced:      this._checkSRI(),
        timestamp:        new Date().toISOString(),
      };
    }

    /** Check whether SRI integrity attributes are present on resource tags */
    _checkSRI () {
      ...
      // Only count tags with non-empty integrity values
      const enforced = Array.from(scripts).concat(Array.from(links))
        .filter(el => el.getAttribute('integrity').length > 0);
